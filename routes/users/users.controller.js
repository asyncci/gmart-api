...
exports.addUser = async function (req, res) {
  if ((req.body.email || req.body.phone) && req.body.password) {
    if (req.body.email) {
      ...
      if (validatePhone(phone)) {
        ...
        if (userWithPhone && userWithPhone._id) {
          return res
            .status(500)
            .send({ success: false, error: "Phone number already exist" });
        }
      } else {
        return res
          .status(500)
    ...
    if (!_.isUndefined(req.body.firstName))
      userData.profile["firstName"] = req.body.firstName;
    if (!_.isUndefined(req.body.lastName))
      userData.profile["lastName"] = req.body.lastName;

    await userData.save().then(async (userObj) => {
      var token = jwt.sign(
        {
          user: {
            _id: userObj._id,
...
exports.editUser = async function (req, res) {
  ...
  if (user._id) {
    ...
    if (req.body.email) {
      if (validateEmail(req.body.email)) {
        ...
        if (userWithEmail) {
          return res
            .status(409)
            .send({ success: false, error: "email already exists" });
        } else {
    ...
    if (!_.isUndefined(req.body.lastName))
      userData["profile.lastName"] = req.body.lastName;
    if (!_.isUndefined(req.body.photo))
      userData["profile.photo"] = req.body.photo;
    // update user
  ...
  if (req.body && (req.body.email || req.body.phone) && req.body.password) {
    ...
    if (req.body.email) {
      ...
      if (!user)
        return res
          .status(400)
          .send({ success: false, error: "Email is incorrect" });
    } else if (req.body.phone) {
      const phone = req.body.phone;
      ...
      if (!user)
        return res
          .status(400)
          .send({ success: false, error: "Phone number is incorrect" });
    }
    ...
    // create token
    var token = jwt.sign(
      { user: _.pick(user, ["_id", "email", "phone", "role"]) },
      config.secret,
      { expiresIn: config.expiresIn }
    );
    ...
    return res.status(200).json({
      ...
      data: {
        ..._.pick(user, ["_id", "phone", "role", "profile", "email"]),
        token,
      },
    });
  } else {
    return res
      .status(400)
      .send({ success: false, error: "email/phone and password are required" });
  }
};

// get users with Authtntication
...
exports.getUsers = async function (req, res) {
  try {
    ...
    if (req.query.search) {
      query["$or"] = [
        {
          "profile.firstName": new RegExp(
            ".*" + req.query.search.trim().replace(/(\W)/g, "\\$1") + ".*",
            "i"
          ),
        },
        {
          "profile.lastName": new RegExp(
            ".*" + req.query.search.trim().replace(/(\W)/g, "\\$1") + ".*",
            "i"
          ),
        },
...
exports.getUserDetail = async function (req, res) {
  ...
  if (user._id) {
    try {
      await User.findOne({ _id: user._id }, { password: 0, token: 0 })
        .populate("addresses", "_id name phone address city state zipCode")
        .then(async (user) => {
...
exports.getTempUserId = async function (req, res) {
  return res
    .status(200)
    .json({ success: true, userId: new mongoose.Types.ObjectId() });
};
...
exports.wishlistProduct = async function (req, res) {
  ...
  if (user._id) {
    if (req?.body?.productId) {
      ...
      var index = -1,
        query = { $push: { wishlist: req.body.productId } };
      if (userObj && userObj.wishlist && userObj.wishlist.length > 0)
        index = _.findIndex(
          userObj.wishlist,
          (productId) => productId?.toString() === req.body.productId.toString()
        );
...
exports.getWishlistProducts = async function (req, res) {
  ...
  if (user._id) {
    const userObj = await User.findOne(
      { _id: user._id },
      { _id: 0, wishlist: 1 }
    ).populate("wishlist", "_id name price specialPrice photos");
    return res
      .status(200)
      .send({
        success: true,
        products: userObj && userObj.wishlist ? userObj.wishlist : [],
      });
  } else {
    return res
      .status(401)
      ...
      if (req?.params?.id) {
        ...
        if (totalOrder > 0)
          return res
            ...
            .send({
              success: false,
              message: "user having order already, It can not be delete",
            });
        ...
        return res
          .status(400)
          .send({ success: false, error: "userId required" });
      }
    } else {
      return res
        .status(401)
        .send({ success: false, error: "only admin can delete user" });
    }
  } else {
    return res
      .status(401)
...
exports.getDashboardData = async function (req, res) {
  ...
  if (user._id) {
    if (user.role && user.role === "admin") {
      ...
      //delivery limit
      const deliveryLimit = await Setting.findOne({
        type: "free_order_delivery_limit",
      });
      if (deliveryLimit && !_.isUndefined(deliveryLimit.value))
        dashboardData["deliveryLimit"] = deliveryLimit.value;

      //delivery charge
      const deliveryCharge = await Setting.findOne({ type: "delivery_charge" });
      if (deliveryCharge && !_.isUndefined(deliveryCharge.value))
        dashboardData["deliveryCharge"] = deliveryCharge.value;

      //today orders
      const startTime = moment().startOf("day");
      const endTime = moment().endOf("day");
      dashboardData["todayOrder"] = await Order.find({
        status: { $ne: "cancelled" },
      ...
      dashboardData["monthOrderTotal"] = _.reduce(
        _.pluck(monthOrder, "totalPrice"),
        function (item, num) {
          return item + (num ? num : 0);
        },
        0
      );
      ...
      return res
        ...
        .send({
          success: false,
          error: "only admin can access dashboard data",
        });
    }
  } else {
    return res
